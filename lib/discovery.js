      ...
      if (sensorData && name) {
        ...
        if (entered) {
          ...
          ha_config.forEach((cfg) => {
            ...
            const topic = `${cfg.type}/${name}/${cfg.object_id}/config`;
            const payload = cfg.discovery_payload;
            payload.state_topic = `${config.base_topic}/${name}/state`;
        ...
        mqtt.send(`${config.base_topic}/${name}/state`, JSON.stringify(sensorData));
      }
    }
  }
}
